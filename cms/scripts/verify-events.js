const http = require('http');
const TOKEN = '';

const req = http.request({
  hostname: 'localhost', port: 1337,
  path: '/api/events?populate=*&pagination[limit]=100',
  method: 'GET',
  headers: { 'Authorization': `Bearer ${TOKEN}` }
}, res => {
  let d = '';
  res.on('data', c => d += c);
  res.on('end', () => {
    const j = JSON.parse(d);
    j.data.forEach(e => {
      const fields = ['background', 'highlights', 'partners', 'itinerary', 'recommendedFor', 'serviceAdvantage', 'contactInfo'];
      const filled = fields.filter(f => e[f] && e[f].length > 0);
      console.log(`[${e.id}] ${e.title} | ${e.category}`);
      console.log(`  ✅ 详情字段: ${filled.join(', ')}`);
    });
  });
});
req.end();
